...
if boltz1:
    ...
    # For BoltzGen, plddt is in the same NPZ file as PAE
    if os.path.exists(pae_file_path):
        data_plddt=np.load(pae_file_path)
        if 'plddt' in data_plddt:
            plddt_boltz1=np.array(100.0*data_plddt['plddt'])
            # Remove batch dimension if present (BoltzGen outputs shape (1, N))
            if plddt_boltz1.ndim == 2 and plddt_boltz1.shape[0] == 1:
                plddt_boltz1 = plddt_boltz1[0]
    ...
    elif os.path.exists(plddt_file_path):
    ...
    if os.path.exists(pae_file_path):
        ...
        # Remove batch dimension if present (BoltzGen outputs shape (1, N, N))
        if pae_matrix_boltz1.ndim == 3 and pae_matrix_boltz1.shape[0] == 1:
            pae_matrix_boltz1 = pae_matrix_boltz1[0]
